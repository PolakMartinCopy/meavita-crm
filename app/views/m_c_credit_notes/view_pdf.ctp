...
$tcpdf->Cell(190, 0, 'Opravný daňový doklad č. ' . $credit_note['MCCreditNote']['code'], 0, 0, 'R', false);
...
$date_of_issue = explode(' ', $credit_note['MCCreditNote']['date_of_issue']);
...
$tcpdf->Cell(90, 0, 'Datum splatnosti: ' . db2cal_date($credit_note['MCCreditNote']['due_date']), 0, 1, 'L', false);

$taxable_event_info = 'Datum zdanitelného plnění: ' . db2cal_date($date_of_issue);

$tcpdf->SetFont($textfont,'', 8);
$tcpdf->Cell(100, 0, 'Variabilní symbol: ' . $credit_note['MCCreditNote']['code'], 0, 0, 'L', false);
...
$note = "";
if (!empty($credit_note['MCCreditNote']['note'])) {
	$note = $credit_note['MCCreditNote']['note'];
}
$tcpdf->SetFont($textfont,'', 8);
...
foreach ($credit_note['MCTransactionItem'] as $transaction_item) {
	$tbl .= "
	<tr>
...
$tcpdf->SetFont($textfont, '', 8);
$tcpdf->Cell(190, 0, 'Základ: ' . number_format(-$credit_note['MCCreditNote']['amount'], 2, ',', ' ') . ' Kč', 0, 1, 'R', false);
$tcpdf->Cell(190, 0, 'Daň: ' . number_format(-($credit_note['MCCreditNote']['amount_vat'] - $credit_note['MCCreditNote']['amount']), 2, ',', ' ') . ' Kč', 0, 1, 'R', false);
$tcpdf->SetFont($textfont, 'B', 8);
$tcpdf->Cell(190, 0, 'Celkem k úhradě: ' . number_format(-$credit_note['MCCreditNote']['amount_vat'], 2, ',', ' ') . ' Kč', 0, 1, 'R', false);
...
echo $tcpdf->Output('medical_corp_credit_note_' . $credit_note['MCCreditNote']['code'] . '.pdf', 'D');
