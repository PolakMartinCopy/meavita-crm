...
$tcpdf->Cell(190, 0, ($invoice['Language']['shortcut'] == 'cs' ? 'Faktura č. ' : 'Invoice ') . $invoice['MCInvoice']['code'], 0, 0, 'R', false);
...
if ($invoice['Language']['shortcut'] == 'cs') {
	...
	$tcpdf->Cell(90, 0, 'Datum splatnosti: ' . db2cal_date($invoice['MCInvoice']['due_date']), 0, 1, 'L', false);
	$tcpdf->SetFont($textfont,'', 8);
	
	$taxable_event_info = 'Datum zdanitelného plnění: ' . db2cal_date($date_of_issue);
	$tcpdf->Cell(100, 0, 'Variabilní symbol: ' . $invoice['MCInvoice']['code'], 0, 0, 'L', false);
	$tcpdf->Cell(90, 0, $taxable_event_info, 0, 1, 'L', false);	
} else {
	$tcpdf->Cell(100, 0, 'Bank name: Fio banka, a.s.', 0, 0, 'L', false);
	$tcpdf->Cell(90, 0, 'Order no.: ' . $invoice['MCInvoice']['order_number'], 0, 1, 'L', false);
...
$note = "";
if (!empty($invoice['MCInvoice']['note'])) {
	$note = $invoice['MCInvoice']['note'];
}
$tcpdf->SetFont($textfont,'', 8);
...
if ($invoice['Language']['shortcut'] == 'cs') {
	$tcpdf->SetFont($textfont, '', 8);
	$tcpdf->Cell(190, 0, 'Základ: ' . number_format($invoice['MCInvoice']['amount'], 2, ',', ' '). ' ' . $invoice['Currency']['shortcut'], 0, 1, 'R', false);
	$tcpdf->Cell(190, 0, 'Daň: ' . number_format($invoice['MCInvoice']['amount_vat'] - $invoice['MCInvoice']['amount'], 2, ',', ' '). ' ' . $invoice['Currency']['shortcut'], 0, 1, 'R', false);
}

if ($invoice['Language']['shortcut'] == 'cs') {
	$result_price_vat = ceil($invoice['MCInvoice']['amount_vat']);
	if ($result_price_vat != $invoice['MCInvoice']['amount_vat']) {
		$tcpdf->SetFont($textfont, '', 8);
		$tcpdf->Cell(190, 0, 'Zaokrouhlení: ' . number_format($result_price_vat - $invoice['MCInvoice']['amount_vat'], 2, ',', ' ') . ' ' . $invoice['Currency']['shortcut'], 0, 1, 'R', false);
	}
	$tcpdf->SetFont($textfont, 'B', 8);
	$tcpdf->Cell(190, 0, 'Celkem k úhradě: ' . number_format($result_price_vat, 2, ',', ' ') . ' ' . $invoice['Currency']['shortcut'], 0, 1, 'R', false);
} else {
	$tcpdf->SetFont($textfont, 'B', 8);
	$tcpdf->Cell(190, 0, 'Total price without VAT: ' . number_format($invoice['MCInvoice']['amount'], 2, ',', ' '). ' ' . $invoice['Currency']['shortcut'], 0, 1, 'R', false);
}
...
echo $tcpdf->Output('medical_corp_invoice_' . $invoice['MCInvoice']['code'] . '.pdf', 'D');
