...
$tcpdf->Cell(190, 0, 'Faktura č. ' . $c_s_m_c_purchase['CSMCPurchase']['code'], 0, 0, 'L', false);
...
$tcpdf->Cell(90, 0, 'MeaVita s.r.o.', 0, 1, 'L', false);

$rep_street_info = 'Cejl 37/62';
$rep_city_info = 'Brno, 602 00';
$rep_ico_info = 'IČ: 29248400';
$rep_dic_info = 'DIČ: CZ29248400';
...
$date_of_issue = explode(' ', $c_s_m_c_purchase['CSMCPurchase']['date_of_issue']);
...
$tcpdf->Cell(100, 0, 'Číslo účtu: DOPLNIT CISLO UCTU', 0, 0, 'L', false);
$tcpdf->SetFont($textfont, 'B', 8);
$tcpdf->Cell(90, 0, 'Datum splatnosti: ' . db2cal_date($c_s_m_c_purchase['CSMCPurchase']['due_date']), 0, 1, 'L', false);
$tcpdf->SetFont($textfont,'', 8);
	
$taxable_event_info = 'Datum zdanitelného plnění: ' . db2cal_date($date_of_issue);
$tcpdf->Cell(100, 0, 'Variabilní symbol: ' . $c_s_m_c_purchase['CSMCPurchase']['code'], 0, 0, 'L', false);
...
foreach ($c_s_m_c_purchase['CSMCTransactionItem'] as $transaction_item) {
	$tbl .= '
	<tr>
...
$tcpdf->SetFont($textfont, '', 8);
$tcpdf->Cell(190, 0, 'Základ: ' . number_format($c_s_m_c_purchase['CSMCPurchase']['amount'], 2, ',', ' '). ' Kč', 0, 1, 'R', false);
$tcpdf->Cell(190, 0, 'Daň: ' . number_format($c_s_m_c_purchase['CSMCPurchase']['amount_vat'] - $c_s_m_c_purchase['CSMCPurchase']['amount'], 2, ',', ' '). ' Kč', 0, 1, 'R', false);

$round = round($c_s_m_c_purchase['CSMCPurchase']['amount_vat']);
$round_diff = $round - $c_s_m_c_purchase['CSMCPurchase']['amount_vat'];
$round_info = 'Zaokrouhlení: ' . number_format($round_diff, 2, ',', ' ') . ' Kč';
$tcpdf->Cell(190, 0, $round_info, 0, 1, 'R', false);

$tcpdf->SetFont($textfont, 'B', 8);

$tcpdf->Cell(190, 0, 'Celkem k úhradě: ' . number_format($c_s_m_c_purchase['CSMCPurchase']['amount_vat'], 2, ',', ' ') . ' Kč', 0, 1, 'R', false);
...
echo $tcpdf->Output('faktura_medicalcorp_meavita_' . $c_s_m_c_purchase['CSMCPurchase']['id'] . '.pdf', 'D');
