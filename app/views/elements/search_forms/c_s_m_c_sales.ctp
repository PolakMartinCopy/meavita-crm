...
<?php
	...
	if ( isset($this->data['CSMCSale']) ){
		$hide = '';
	}
?>
<div id="search_form"<?php echo $hide?>>

	<?php echo $form->create('CSMCSale', array('url' => array('controller' => 'c_s_m_c_sales', 'action' => 'index')))?>
	<table class="left_heading">
		...
		<tr>
			<th>Datum od</th>
			<td><?php echo $this->Form->input('CSMCSale.date_from', array('label' => false))?></td>
			<th>Datum do</th>
			<td><?php echo $this->Form->input('CSMCSale.date_to', array('label' => false))?></td>
			<td colspan="2">&nbsp;</td>
		</tr>
		<tr>
			<td colspan="6">
				<?php echo $html->link('reset filtru', array('controller' => $this->params['controller'], 'reset' => 'c_s_m_c_sales')) ?>
			</td>
		</tr>
	</table>
	
	<?php
		echo $form->hidden('CSMCSale.search_form', array('value' => 1));
