...
$tcpdf->Cell(90, 0, 'Datum splatnosti: ' . db2cal_date($b_p_c_s_rep_sale['BPCSRepSale']['due_date']), 0, 1, 'L', false);
$tcpdf->SetFont($textfont,'', 8);
	
$taxable_event_info = 'Datum zdanitelného plnění: ' . db2cal_date($date_of_issue);
$tcpdf->Cell(100, 0, 'Variabilní symbol: ' . $b_p_c_s_rep_sale['BPCSRepSale']['code'], 0, 0, 'L', false);
...
$tcpdf->SetFont($textfont, '', 8);
$tcpdf->Cell(190, 0, 'Základ: ' . number_format($b_p_c_s_rep_sale['BPCSRepSale']['amount'], 2, ',', ' '). ' Kč', 0, 1, 'R', false);
$tcpdf->Cell(190, 0, 'Daň: ' . number_format($b_p_c_s_rep_sale['BPCSRepSale']['amount_vat'] - $b_p_c_s_rep_sale['BPCSRepSale']['amount'], 2, ',', ' '). ' Kč', 0, 1, 'R', false);

$round = round($b_p_c_s_rep_sale['BPCSRepSale']['amount_vat']);
$round_diff = $round - $b_p_c_s_rep_sale['BPCSRepSale']['amount_vat'];
